<?php
	...
	class user_select
	{
		...
		function retrieve_bind($con, $a, $b){
	 	...
	 	function retrieve_tables($con){
			...
			$this->sql = "SHOW TABLES";
 			...
 			if(!$this->bind){
	 			...
	 			$this->bind->execute();
	 			$this->bind->bind_result($this->col);
				while ($this->bind->fetch()) {
	 	...
	 	function retrieve_columns($con, $a){
	 		$this->result = array(); 
 			$this->check = new user_connect($con);
			$this->sql = "SHOW FIELDS IN $a WHERE EXTRA = ''";
 			$this->bind = $this->check->connect->prepare($this->sql);
 			$b = array();
 			for ($i=0;$i < 6;$i++) {
					$this->col[] = &$b[$i];	
